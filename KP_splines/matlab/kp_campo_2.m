...
% Archivo de salida
name = sprintf('./res20170601/E_vs_B-2.dat');
...
fprintf(file, '# kappa = [%f, %f, %f, %f, %f, %f, %f, %f]\n', kappa);
  ...
  for m1 = -m_angular:m_angular
    for m2 = -m_angular:m_angular
      ...
      B = sqrt(3)*hbar^2*gamma(3)/m0*k_z*kronDel(m2-m1-1,0)*...
          sparse(complex(B, -0.5/lB^2*full(rho1)));
