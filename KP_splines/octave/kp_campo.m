...
bcampo_f = 25; % Campo final en Teslas
num_puntos_campo = 240;

N_dim_H = 8*(2*m_angular+1)*N_base;

% Parametros del material (ahora uso de GaAs)
Eg = 1.519/eV; % atomic units
Ep = 25.7/eV; % atomic units
delta = 0.33/eV; % atomic units
...
% kappa = [0.0842, 0.0842, -1.0625, -1.0625, -1.0625, -1.0625, -1.0625, -1.0625];
...
% Archivo de salida
name = sprintf('./resultados/E_vs_B.dat');
...
for ind_campo = 1:num_puntos_campo
  ...
  for m1 = -m_angular:m_angular
    for m2 = -m_angular:m_angular
      ...
      PP = -complex(0,P*kronDel(m2-m1+1,0)*(D - m2*rho_1) + 0.5/lB^2*rho1);

      % Calculo el bloque de P^-
      PM = -complex(0,P*kronDel(m2-m1-1,0)*(D + m2*rho_1) - 0.5/lB^2*rho1);

      % Calculo el bloque de P^z
      Pz = kronDel(m1,m2)*P*k_z*S;

      % Calculo el bloque B
      B = sqrt(3)*hbar^2*gamma(3)/m0*k_z*kronDel(m2-m1-1,0)*complex((D + m2*rho_1), ...
          -0.5/lB^2*rho1);
