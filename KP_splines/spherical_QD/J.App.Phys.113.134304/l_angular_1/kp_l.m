...
l_angular = 1; % Momento angular total
mu_B = 5.7883818066e-5; % Borh magneton in eV/T
Rc_i = 5/a0; % atomic units
Rc_f = 5/a0; % atomic units
N_dim_H = 8*N_base*(l_angular + 1);
num_puntos_Rc = 1;
...
% Archivo de salida
name = sprintf('./resultados/E_vs_Rc.dat');
...
% Hago una copia para las integrales en theta
u = x; w_u = w;
...
% Primero calculo la matriz de solapamiento en r
...
for Rc = Rc_vec
  ...
  for l1 = 0:l_angular
    for l2 = 0:l_angular
      ...
      pl1 = legendre(theta, l1); % P_{l_1}(u)
      pl2 = legendre(theta, l2); % P_{l_2}(u)
      pl2p1 = legendre(theta, l2+1); % P_{l_2 + 1}(u)
      pl2p2 = legendre(theta, l2+2); % P_{l_2 + 2}(u)

      dz1 = bs*(w.*P0.*x.^2.*dbs')*kronDel(l1,l2);
      dz2 = bs*(w.*P0.*x.*bs');
